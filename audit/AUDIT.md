...
完整阅读两篇 PDF 的正文、数学定义、实验与结论；证明用于理解假设和适用范围，不重建证明。提取文本并视觉检查全部图页，尤其 thesis PDF 26–29、38、41–42、48、50–52、55、59、62、65、69–70、74 和 extension 3、7–8、10、12 页。thesis 以下位置使用印刷页，PDF 页通常加 6。初次审计包含 thesis 23 图、extension 6 图及补充实验，作者随后新增ACC 3图，当前 `FIGURE_MANIFEST.md` 共32图；两文没有需另行复现的编号数据表或编号算法表。正文案例和算法规则也纳入审计。
...
L ifac2023 / cdc2023 各五个 case 是纯几何/向量场展示，不含 ode45 时间轨迹。这些文件仅为候选参考，不能认定原图入口；初次审计按代码行及图形推定 T3.5/T3.6 子图 a,b,c,d,e 对应 case1,2,4,5,3；此对应为有证据的 agent 判断，不是原文件声明。按 (11,12,21,22) 排列的 own-row determinant：
...
| C10（作者已澄清） | 特定条件下无法满足全部设计要求；满足theorem条件则保证成立 | 按ACC Theorem2与可行性必要条件区分表述，禁止写成一般不能实现激励 |
...
| 核查 | 结果 |
|---|---|
| paper-all（历史无效转录） | 旧数组计算不是正确的论文对应；撤回作为Q1证据，见AUTHOR_REVIEW |
...
| Phase 0 验收项 | 证据 |
|---|---|
| 每个相关论文实验均有去向 | FIGURE_MANIFEST 32图（含新增ACC3图）、非图正文案例与补充实验；缺源码项不静默省略 |
