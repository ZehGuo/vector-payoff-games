...
| D51 | GitHub公开入口采用README 7张代表图＋RESULTS全部18张技术图的两层结构；README直接解释问题与Nash-set核心，audit降为可选验证深入链接 | RR-05任务与PUBLICATION_BLUEPRINT；RR-01至04 tracked产物 | `README.md`、`docs/RESULTS.md`、`docs/CONCEPTS_AND_NOTATION.md`、`docs/PROVENANCE.md` | 新读者不需先读PROJECT_STATE/AUDIT；每张公开图必须保留caption、alt和do-not-infer边界 | 作者任务＋已完成读者审阅 |
| D52 | 发布状态与读者入口完成分开：P1公开图源已由D49关闭；当时许可证、正式citation元数据和公开paper/DOI链接仍须作者提供 | RR-03结果与RR-05授权边界 | README发布阻碍、PROVENANCE | 当时不创建LICENSE/CITATION.cff；此门槛后来由D53–D55关闭 | 作者边界＋历史状态 |
| D53 | 作者确认代码采用BSD-3-Clause，原创文档和repository-generated figures采用CC BY 4.0；不覆盖论文PDF、第三方材料、legacy archive或未分发内容 | 2026-09-20作者批准 | `LICENSE`、`LICENSE-DOCUMENTATION`、README | 允许公开复用，同时保留科学内容署名和第三方权利边界 | 作者决定 |
| D54 | 软件以Zehui Guo为作者、`Vector-Payoff Games: From Nash-Set Geometry to Incentive Design`为标题，首发`v1.0.0`；模块使用者另引对应论文 | 主页publication元数据与作者批准 | `CITATION.cff`、`docs/RELATED_PUBLICATIONS.md` | GitHub显示软件引用；ECC 2026 DOI/page range后续仅作metadata更新 | 作者决定＋公开记录 |
| D55 | 公开仓库使用`ZehGuo/vector-payoff-games`；发布仓库不等于部署个人主页 | 作者“如果可以上传到GitHub就搞”授权 | GitHub main/v1.0.0；`publication/homepage/` | 可创建公开repo并推送；不得顺带修改或部署真实主页仓库 | 作者授权 |
